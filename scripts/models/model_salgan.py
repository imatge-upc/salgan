...
class ModelSALGAN(Model):
    def __init__(self, w, h, batch_size=10, G_lr=0.001, D_lr=0.01, alpha=1/20.):
...
#        + 1e-4 * lasagne.regularization.regularize_network_params(self.net[output_layer_name], lasagne.regularization.l2)
        ...
        D_obj = lasagne.objectives.binary_crossentropy(T.concatenate([disc_lab, disc_gen], axis=0),T.concatenate([ones, zeros], axis=0)).mean() + 1e-3 * lasagne.regularization.regularize_network_params(self.discriminator['prob'], lasagne.regularization.l2)

        G_obj_d = lasagne.objectives.binary_crossentropy(disc_gen, T.ones(disc_lab.shape)).mean() + 1e-3 * lasagne.regularization.regularize_network_params(self.net[output_layer_name], lasagne.regularization.l2)
