...
class ModelBCE(Model):
	...
	output_var_pooled = T.signal.pool.pool_2d(self.output_var, (16, 16), mode="average_exc_pad", ignore_border=True)
        bce = lasagne.objectives.binary_crossentropy(prediction, output_var_pooled).mean() + regterm * lasagne.regularization.regularize_network_params(self.net[output_layer_name], lasagne.regularization.l2)
        train_err = bce
    	...
    	test_loss = lasagne.objectives.binary_crossentropy(test_prediction,output_var_pooled).mean()
	test_acc = lasagne.objectives.binary_jaccard_index(test_prediction,output_var_pooled).mean()
